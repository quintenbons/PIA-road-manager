...
@timing
def extract_connex_roads_from_csv(roads, city_name):
    G = nx.Graph()
    ...
    road_complex = []
    filename = os.path.join(BUILD_DIR, city_name+LARGEST_CONNECTED_COMPONENT)
...
def read_raw_data(city_name):
    filename = os.path.join(BUILD_DIR, city_name+FINAL_ROADS_FILE)
...
@timing
def process_road_data(city_name, GENERATE_CSV=True):
    ...
    if GENERATE_CSV:
        print("Saving data...")
        save_to_csv(city_name+ROADS_FILE, [[point for point in road] for road in roads])
        roads = simplify_roads_csv(city_name+ROADS_FILE, city_name+SIMPLIFIED_ROADS_FILE)

        save_to_csv(city_name+INTERSECTIONS_FILE, intersections)

        print("Extract largest connected component...")
        roads = extract_connex_roads_from_csv(roads, city_name)
        # write roads to file
        save_to_csv(city_name+FINAL_ROADS_FILE, [[point for point in road] for road in roads])
        
        raw_data = read_raw_data(city_name)
        nodes, routes = create_nodes_and_routes(raw_data)
        write_to_csv(nodes, routes, city_name+MAP_FILE)
    else:
